...
const Index = ({country, user}) => {
  ...
  const [datas, setDatas] = useState(country);

  const filteredTypes = (datas || []).filter(item => 
    //item.amount.toLowerCase().includes(filterText.toLowerCase()) ||
    item.name_fr.toLowerCase().includes(filterText.toLowerCase()) ||
    item.name_en.toLowerCase().includes(filterText.toLowerCase()) ||
    item.phone.toLowerCase().includes(filterText.toLowerCase()) ||
    item.alpha.toLowerCase().includes(filterText.toLowerCase()) ||
    item.alph.toLowerCase().includes(filterText.toLowerCase()) ||
    item.iso.toLowerCase().includes(filterText.toLowerCase())
  );
  ...
  function handleDeleteCountry(id, name) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.get('countries/destroy/'+id).then(res => {
          if (res.status == 200) {
            setDatas(res.data);
  ...
  // store function of PostContoller
  function handleActivesCountry(id) {
    axios.get('countries/actives/'+id).then(res => {
      if (res.status == 200) {
        setDatas(res.data);
      }
     }).catch(err => {
  ...
  return (
    <Container fluid className="py-4">
      ...
      <Row className="mb-4 align-items-center">
        <Col>
          <h2 className="fw-bold">Gestion des pays</h2>
        </Col>
        <Col className="text-end">
            <a href="countries/create" className="px-2">
              <Button variant="primary">
                <FaPlus className="me-2" /> Ajouter un pays
              </Button>
            </a>
        ...
        <Card.Body>
          ...
          <thead>
            <tr>
              ...
              <th onClick={() => handleSort("alpha")} style={{ cursor: "pointer" }}>Alpha 3</th>
              <th onClick={() => handleSort("alph")} style={{ cursor: "pointer" }}>Alpha 2</th>
              <th onClick={() => handleSort("iso")} style={{ cursor: "pointer" }}>Code Iso</th>
              <th onClick={() => handleSort("name_fr")} style={{ cursor: "pointer" }}>Nom Français</th>
              <th onClick={() => handleSort("name_en")} style={{ cursor: "pointer" }}>Nom Anglais</th>
          ...
          <tbody>
            {paginatedTypes.map((item, index) => (
              <tr key={index}>
                ...
                <td>
                                    {<a type="button" onClick={() => handleActivesCountry(item.id)}>{item.actives == 0 ? <OverlayTrigger
                                      placement='bottom'
                                      overlay={
                                  ...
                                  {user.role_id >= 4 ? <a type="button" onClick={() => handleDeleteCountry(item.id, item.name_fr)}><OverlayTrigger
                                    placement='bottom'
                                    overlay={
                                      <Tooltip id={`tooltip-bottom`}>
                                        Supprimer
