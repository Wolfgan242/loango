...
const Create = ({institutions, user}) => {
  ...
  const [establishment, seteEstablishment] = useState({
  ...
  const validateForm = () => {
    let formErrors = {};
    if (!establishment.description) formErrors.reference = "Veuillez remplir ce champ.";
    if (!establishment.website) formErrors.website = "Veuillez remplir ce champ.";
    if (!establishment.address) formErrors.address = "Veuillez remplir ce champ.";
    if (!establishment.phone) formErrors.phone = "Veuillez remplir ce champ.";
    if (!establishment.email) formErrors.email = "Veuillez remplir ce champ.";
    if (!establishment.name) formErrors.name = "Veuillez remplir ce champ.";
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      setLoading(true);
      axios.post('store', establishment).then(res => {
          if (res.data == 'saved') {
              setMessage('Etablishment créée!');
              setVariant('success');
              setLoading(false);
              setAlert(true);
          }else{
  ...
  const handleChangeInstitution = selected => {
    selected ? establishment.institution_id = selected.value : "";
  ...
  const handleChange = (e) => {
    seteEstablishment({ ...establishment, [e.target.id]: e.target.value });
  };
        ...
        <Col lg={10}>
          <Card className="shadow-lg border-0 p-4">
            <Card.Body>
              <Row>
                <Col>
                  <h3 className="fw-bold">Ajouter un établissement !</h3>
                </Col>
